...
static uint8 com_leave(ALARM_POINT_NODE *pPoint, uint8 flag)
{
	uint8 voice_index;
	...
	voice_index = camera_voice;

	pPoint->status = ALARM_POINT_STATUS_EXIT;

	VoicePlayCommon(voice_index, TTS_PLAY_LEVEL_MID, VOICE_DATA_FLAG_NULL);

	return 0;
}

static void com_timer(ALARM_POINT_NODE *pPoint, uint8 flag)
{
...
POINT_PROCESS_TYPE gCommonPoint =
{
	com_enter,
	com_leave,
	com_timer,
